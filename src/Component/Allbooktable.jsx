...
export default function Allbooktable({ book }) {
    return (
        <tr className="border-b border-secondary  transition-all duration-200 cursor-pointer">
            ...
            <td className="py-2 px-3 md:py-4 md:px-5 text-yellow-600 text-sm md:text-base font-semibold flex items-center gap-1">
                <FaStar className="text-yellow-500" /> {book.rating} / 5
            </td>
            <td className="py-2 px-3 md:py-4 md:px-5">
                <Link
                    to={`/details/${book._id}`}
                    className="bg-linear-to-r from-blue-500 to-indigo-600 text-[12px] md:text-sm text-white px-4 py-1.5 rounded-full font-semibold shadow hover:from-blue-600 hover:to-indigo-700 transition-all duration-300"
                >
                    View
                </Link>
            </td>
